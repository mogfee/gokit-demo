...
import (
	...
	"github.com/mogfee/gokit-demo/demo/spiderserver-service/handlers"
...
// Config contains the required fields for running a service
type Config struct {
	HTTPAddr  string
	DebugAddr string
	GRPCAddr  string
...
func NewEndpoints() svc.Endpoints {
	// Business domain.
	var service pb.SpiderServerServer
	{
	...
	// Endpoint domain.
	var (
		parselistEndpoint   = svc.MakeParseListEndpoint(service)
		parsedetailEndpoint = svc.MakeParseDetailEndpoint(service)
	)
...
// Run starts a new http service, gRPC service, and a debug service with the
...
func Run(cfg Config) {
	...
	go func() {
		...
		s := grpc.NewServer()
		pb.RegisterSpiderServerServer(s, srv)
